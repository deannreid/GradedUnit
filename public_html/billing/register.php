...
if($_POST && isset($_SESSION['token']) && ($_SESSION['token'] == $_POST['token']) && $set->register && $captcha) {
  ...
  if(!isset($name[3]))
    $page->error = "Username too short!";

  if(isset($name[30]))
    $page->error = "Username too long!";
  ...
  if($db->getRow("SELECT `userid` FROM `".MLS_PREFIX."users` WHERE `email` = ?s", $email))
    ...
    if($set->email_validation == 1) {
      ...
      $msg = "Hello ".$options->html($name).",<br> Thank you for choosing to be a member of out company.<br/><br/> To confirm your account <a href='$link'>click here</a>.<br>If you can't access copy this to your browser<br/>$link  <br><br>Regards, <br> XenoHost Staff <br><br><small>Note: Dont reply to this email. If you got this email by mistake then ignore this email.</small>";
